...
void PieceManager::blockReceived(std::string peerId, int pieceIndex, int blockOffset, std::string data)
{
    ...
    //std::cout<<"Received block "<<blockOffset<<" for piece "<<pieceIndex<<" from peer "<<peerId.c_str()<<std::endl;
    // TODO: std::cout
...
{
    std::this_thread::sleep_for(std::chrono::microseconds((int)pow(10, 6)));
    while (!isComplete())
    {
        displayProgressBar();
        // Resets the number of pieces downloaded to 0
        piecesDownloadedInInterval = 0;
        std::this_thread::sleep_for(std::chrono::microseconds(int(PROGRESS_DISPLAY_INTERVAL * pow(10, 6))));
    }
}
...
void PieceManager::displayProgressBar()
{
    ...
    info << std::fixed << std::setprecision(6) << avgDownloadSpeedInMBS << " MB/s, ";
