 ...
 */
    std::vector<Peer*> PeersRetriever::retrievePeers(unsigned long bytesDownloaded)
    {
 ...
 * other two files.
 */
    std::vector<Peer*> PeersRetriever::decodeResponse(std::string response) {
        //LOG_F(INFO, "Decoding tracker response...");
        bencode::data decodedResponse = bencode::decode(response);

        bencode::dict responseDict =std::get<bencode::dict>(decodedResponse);
        bencode::data peersValue = responseDict["peers"];
        if (peersValue.valueless_by_exception())
        ...
        if (peersValue.index() == 1) // int 0 str 1 list 2 dict 3
        {
            ...
            std::string peersString = std::get<bencode::string>(peersValue);
        ...
        else if (peersValue.index() == 2)// int 0 str 1 list 2 dict 3
        {
            bencode::list peerList = std::get<bencode::list>(peersValue);
            for (auto &item : peerList)
            {
                // Casts each item to a dictionary
                bencode::dict peerDict = std::get<bencode::dict>(item);

                // Gets peer ip from the dictionary
                bencode::data tempPeerIp = peerDict["ip"];

                if (tempPeerIp.valueless_by_exception())
                    throw std::runtime_error("Received malformed 'peers' from tracker. [Item does not contain key 'ip']");

                std::string peerIp = std::get<bencode::string>(tempPeerIp);
                // Gets peer port from the dictionary
                bencode::data tempPeerPort = peerDict["port"];
                if (tempPeerPort.valueless_by_exception())
                    throw std::runtime_error("Received malformed 'peers' from tracker. [Item does not contain key 'port']");
                int peerPort = (int) std::get<bencode::integer>(tempPeerPort);
